...
public class QuoteViewsProvider {
	...
	public static View getQuoteView(final Context context,
		...
		if (position == null) {
			...
			if (LanguageController.getCurrentLanguage() == LanguageController.ENG) {
		...
		int item = randInt.nextInt(show.getQuotesList(language).size());
		while (onScreenQuotesList.contains(item)) {
			item = randInt.nextInt(show.getQuotesList(language).size());
		}
		onScreenQuotesList.add(item);
